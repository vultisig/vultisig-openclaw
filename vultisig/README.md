...
Edit `vultisig/config.yml` (copied from `config.example.yml`):
...
```bash
./vultisig/status.sh                        # quick health check
...
OpenClaw source files are untouched — upstream merges stay clean.
